 ...
 * Provides unified interface for worker communication and load balancing
 */
...
class WorkerManager {
    ...
    async init() {
        ...
        try {
            // Use relative URLs for workers to avoid CORS issues
            const workerPromises = [
                this.initWorker('conversation', './static/js/workers/conversationWorker.js'),
                this.initWorker('storage', './static/js/workers/storageWorker.js'),
                this.initWorker('message', './static/js/workers/messageWorker.js')
            ];
    ...
    async initWorker(type, scriptUrl) {
        return new Promise((resolve) => {
            try {
                console.log(`Creating worker ${type} from ${scriptUrl}`);
                ...
                worker.onerror = (error) => {
                    if (!isReady) {
                        console.error(`Worker ${type} failed to initialize:`, error);
                    }
                };
                ...
                setTimeout(() => {
                    if (!isReady) {
                        console.error(`Worker ${type} initialization timeout`);
                        worker.terminate();
    ...
    /**
     * Setup event handling for worker
     */
    ...
    handleWorkerError(type, error) {
        console.warn(`Worker ${type} error, attempting recovery...`);
     ...
     */
    async prepareConversationForAPI(conversation, systemPrompt, selectedModel) {
    ...
    async exportConversationAsJSON(conversation) {
        return this.sendToWorker('message', 'exportAsJSON', conversation);
    }

    async exportConversationAsMarkdown(conversation) {
        return this.sendToWorker('message', 'exportAsMarkdown', conversation);
    }
