...
class ChatUI {
    ...
    // Restore conversation data from artifacts
    async restoreConversationFromArtifacts(conversationId) {
        ...
        if (this.artifactServerUnavailable) {
            this.showNotification('Artifact server is unavailable', 'warning');
            return false;
        }
        ...
        try {
            this.showNotification('Restoring conversation from artifacts...', 'info');

            // Process each message
            for (const message of conversation.messages) {
                // Handle attachments array (older format)
                if (message.attachments && Array.isArray(message.attachments)) {
                    for (const attachment of message.attachments) {
                        if (attachment.stripped &&
                            ...
                            // Try to find corresponding artifact ID
                            const artifactId = await this.findArtifactIdForAttachment(attachment, message);
                            if (artifactId) {
                                try {
                ...
                // Handle content array (newer format)
                if (Array.isArray(message.content)) {
                    for (const item of message.content) {
                        if (item.stripped) {
                            ...
                            if (item.type === 'audio' && item.audio?.data === '[LARGE_AUDIO_STRIPPED_USE_ARTIFACT]') {
                                ...
                                if (artifactId) {
                                    try {
                                        ...
                                        console.warn(`Failed to restore audio artifact ${artifactId}:`, error);
                                        failedCount++;
                                    }
                                }
                            } else if (item.type === 'image' && item.image?.data === '[LARGE_IMAGE_STRIPPED_USE_ARTIFACT]') {
                                const artifactId = await this.findArtifactIdForContentItem(item, message);
                                if (artifactId) {
                                    try {
                                        const dataURL = await this.fetchArtifactAsDataURL(artifactId);
                                        ...
                                        console.warn(`Failed to restore image artifact ${artifactId}:`, error);
                                        failedCount++;
                                    }
                                }
                            } else if (item.type === 'file' && item.file?.data === '[LARGE_FILE_STRIPPED_USE_ARTIFACT]') {
                                const artifactId = await this.findArtifactIdForContentItem(item, message);
                                if (artifactId) {
                                    try {
                                        const dataURL = await this.fetchArtifactAsDataURL(artifactId);
                                        ...
                                        console.warn(`Failed to restore file artifact ${artifactId}:`, error);
                                        failedCount++;
                                    }
                                }
                            }
            ...
            // Refresh UI if this is the current conversation
            ...
            if (restoredCount > 0) {
                const message = failedCount > 0 ?
                    ...
                    `Successfully restored ${restoredCount} items from artifacts`;
                this.showNotification(message, failedCount > 0 ? 'warning' : 'success');
                return true;
            } else {
                this.showNotification('No stripped content found to restore', 'info');
                return false;
            }

        } catch (error) {
            console.error('Error during artifact restoration:', error);
            this.showNotification(`Restoration failed: ${error.message}`, 'error');
            return false;
        }
    }

    // Helper method to find artifact ID for attachment (implementation may need customization based on your artifact storage strategy)
    async findArtifactIdForAttachment(attachment, message) {
        // Option 1: Check if there's a stored artifact ID in the attachment
        if (attachment.artifactId) {
            return attachment.artifactId;
        }

        // Option 2: Look for artifact references in the same message content
        if (Array.isArray(message.content)) {
            for (const item of message.content) {
                // Check for artifact references that match this attachment type
                if (item.type === 'image_artifact' && attachment.image_url) {
                    return item.image_artifact?.artifactId;
                }
                if (item.type === 'audio_artifact' && attachment.audio) {
                    return item.audio_artifact?.artifactId;
                }
                if (item.type === 'file_artifact' && (attachment.image_url || attachment.audio)) {
                    return item.file_artifact?.artifactId;
                }
            }
        }

        // Option 3: Check for backup artifact data in the attachment itself
        if (attachment.backup_artifact_id) {
            return attachment.backup_artifact_id;
        }

        // Option 4: Look for artifact IDs in filename metadata
        if (attachment.filename && message.artifact_metadata) {
            const metadata = message.artifact_metadata[attachment.filename];
            if (metadata && metadata.artifactId) {
                return metadata.artifactId;
            }
        }

        return null; // Return null if no artifact ID can be determined
    }

    // Helper method to find artifact ID for content item
    async findArtifactIdForContentItem(item, message) {
        // Option 1: Check if there's a stored artifact ID in the item
        if (item.artifactId) {
            return item.artifactId;
        }

        // Option 2: Check for artifact reference in the same message
        if (Array.isArray(message.content)) {
            for (const contentItem of message.content) {
                if (contentItem.type === 'image_artifact' && item.type === 'image') {
                    return contentItem.image_artifact?.artifactId;
                }
                if (contentItem.type === 'audio_artifact' && item.type === 'audio') {
                    return contentItem.audio_artifact?.artifactId;
                }
                if (contentItem.type === 'file_artifact' && item.type === 'file') {
                    return contentItem.file_artifact?.artifactId;
                }
            }
        }

        // Option 3: Check backup artifact reference
        if (item.backup_artifact_id) {
            return item.backup_artifact_id;
        }

        // Option 4: Look in message-level artifact metadata
        if (item.filename && message.artifact_metadata) {
            const metadata = message.artifact_metadata[item.filename];
            if (metadata && metadata.artifactId) {
                return metadata.artifactId;
            }
        }

        // Option 5: Try to extract from original data if it contains artifact reference
        const originalData = item.audio?.data || item.image?.data || item.file?.data;
        if (originalData && originalData.includes('artifact:')) {
            const match = originalData.match(/artifact:([a-zA-Z0-9-_]+)/);
            if (match) {
                return match[1];
            }
        }
    ...
    // Check if a conversation has stripped content that could be restored
    conversationHasStrippedContent(conversation) {
        if (!conversation.messages) return false;

        for (const message of conversation.messages) {
            // Check attachments array (older format)
            if (message.attachments && Array.isArray(message.attachments)) {
                for (const attachment of message.attachments) {
                    if (attachment.stripped &&
                        (attachment.image_url?.url === '[LARGE_DATA_STRIPPED_USE_ARTIFACT]' ||
            ...
            // Check content array (newer format)
            if (Array.isArray(message.content)) {
                for (const item of message.content) {
                    if (item.stripped &&
    ...
    getFileType(filename) {
        ...
        if (['jpg', 'jpeg', 'png', 'gif', 'bmp', 'svg', 'webp'].includes(extension)) {
            ...
            return 'audio';
        } else if (['js', 'css', 'html', 'htm', 'json', 'xml', 'py', 'java', 'cpp', 'c'].includes(extension)) {
            return 'code';
    ...
    getFileTypeColors(fileType) {
        switch (fileType) {
            case 'pdf':
                return { color: '#dc2626', bgColor: '#fef2f2', borderColor: '#fecaca' };
            case 'document':
                return { color: '#2563eb', bgColor: '#eff6ff', borderColor: '#bfdbfe' };
            case 'spreadsheet':
                return { color: '#059669', bgColor: '#ecfdf5', borderColor: '#a7f3d0' };
            case 'presentation':
                return { color: '#d97706', bgColor: '#fffbeb', borderColor: '#fed7aa' };
            case 'image':
            ...
            case 'audio':
                return { color: '#059669', bgColor: '#ecfdf5', borderColor: '#a7f3d0' };
            case 'archive':
                return { color: '#92400e', bgColor: '#fefbf2', borderColor: '#fde68a' };
            case 'code':
                return { color: '#374151', bgColor: '#f9fafb', borderColor: '#d1d5db' };
            case 'text':
                return { color: '#6b7280', bgColor: '#f9fafb', borderColor: '#e5e7eb' };
            default:
                return { color: '#6b7280', bgColor: '#f9fafb', borderColor: '#e5e7eb' };
        }
    }
    ...
    saveCurrentConversation() {
        if (this.currentConversationId && this.conversations[this.currentConversationId]) {
            ...
            // Check if there are actual changes before updating lastModified
            const oldMessagesJson = JSON.stringify(conversation.messages || []);
            const newMessagesJson = JSON.stringify(this.messages);
            const oldSystemPrompt = conversation.systemPrompt || '';
            const newSystemPrompt = this.systemPrompt || '';

            const hasChanges = oldMessagesJson !== newMessagesJson || oldSystemPrompt !== newSystemPrompt;

            // Store all messages with their full content including attachments
            conversation.messages = this.messages;
            conversation.systemPrompt = this.systemPrompt;  // Save system prompt
            ...
            // Use worker-based save if available, otherwise fallback to direct save
            if (this.workerReady && this.workerManager && this.workerManager.isInitialized) {
                this.saveConversationsViaWorker().catch(error => {
                    console.error('Error saving conversation via worker:', error);
                    // Fallback to direct save if worker fails
                    this.saveConversations().catch(fallbackError => {
    ...
    renderConversationsList() {
        ...
        const groupedConversations = this.groupConversationsByDate(sortedIds);
        ...
        sortedDateKeys.forEach(dateKey => {
            ...
            dateLabel.className = 'conversation-date-label';
            dateLabel.textContent = this.formatDateHeader(dateKey);

            const foldButton = document.createElement('button');
            foldButton.className = 'conversation-fold-button';
            foldButton.innerHTML = '<span class="material-icons">expand_less</span>';
            foldButton.title = 'Toggle conversations';

            dateHeader.appendChild(dateLabel);
            dateHeader.appendChild(foldButton);

            // Create conversations container for this date
            const conversationsContainer = document.createElement('div');
            conversationsContainer.className = 'conversation-date-group';
            // Start with all groups folded by default
            conversationsContainer.classList.add('folded');

            // Add conversations to this date group
            conversations.forEach(id => {
                ...
                // Check if conversation has stripped content that could be restored
                const hasStrippedContent = this.conversationHasStrippedContent(conv);
                ...
                // Add restore button if conversation has stripped content
                if (hasStrippedContent) {
                    const restoreBtn = document.createElement('button');
                    restoreBtn.className = 'restore-btn';
                    restoreBtn.innerHTML = '🔄';
                    restoreBtn.title = 'Restore from artifacts';
                    restoreBtn.onclick = (e) => {
                        e.stopPropagation();
                        this.restoreConversationFromArtifacts(id);
                    };
            ...
            foldButton.onclick = () => {
                conversationsContainer.classList.toggle('folded');
                const icon = foldButton.querySelector('.material-icons');
                if (conversationsContainer.classList.contains('folded')) {
            ...
            // Update fold button icon based on initial state
            const icon = foldButton.querySelector('.material-icons');
            icon.textContent = conversationsContainer.classList.contains('folded') ? 'expand_more' : 'expand_less';

            this.conversationsList.appendChild(dateHeader);
            this.conversationsList.appendChild(conversationsContainer);
        });
    }

    groupConversationsByDate(sortedIds) {
        const groups = {};

        sortedIds.forEach(id => {
            const conv = this.conversations[id];
            const date = new Date(conv.lastModified || conv.timestamp || conv.createdAt || Date.now());
            const dateKey = date.toDateString(); // e.g., "Tue Oct 01 2025"

            if (!groups[dateKey]) {
                groups[dateKey] = [];
            }
            groups[dateKey].push(id);
        });

        // Sort conversations within each date group by lastModified (latest first)
        Object.keys(groups).forEach(dateKey => {
            groups[dateKey].sort((a, b) => {
                const convA = this.conversations[a];
                const convB = this.conversations[b];
                const timeA = convA.lastModified || convA.timestamp || convA.createdAt || 0;
                const timeB = convB.lastModified || convB.timestamp || convB.createdAt || 0;
    ...
    formatDateHeader(dateString) {
        ...
        // For other dates, format as "October 1st 2025"
        const options = {
            year: 'numeric',
            month: 'long',
            day: 'numeric'
    ...
    getCustomRenderer() {
        ...
        renderer.code = function(code, infoString, escaped) {
            ...
            if (validLang === 'mermaid') {
                ...
                // Return a container that will be processed by Mermaid.js
                return `<div class="mermaid-container" data-mermaid-id="${mermaidId}">
                    <button class="mermaid-save-btn" onclick="chatUI.saveMermaidAsPNG('${mermaidId}')">
        ...
        if (typeof content === 'string') {
            ...
            for (const item of content) {
                if (item.type === 'text') {
                    ...
                    // Display file attachment with enhanced visual representation
                    const filename = item.file.filename || 'Unknown file';
                    ...
                    const isStripped = item.file.file_data === '[LARGE_FILE_STRIPPED_USE_ARTIFACT]';

                    if (isPlaceholder || isStripped) {
                        const fileIcon = this.getFileTypeIcon(filename);
                        html += `<div class="message-file placeholder" style="display: inline-flex; align-items: center; padding: 12px; background: #f9f9f9; border: 2px dashed #d1d5db; border-radius: 8px; margin: 8px 0; gap: 12px; min-width: 250px;">
                            <span class="material-icons" style="color: #9ca3af; font-size: 24px;">${fileIcon}</span>
                            <div style="flex: 1;">
                                <div style="font-size: 14px; color: #374151; font-weight: 500; margin-bottom: 2px;">${filename}</div>
                                <div style="font-size: 12px; color: #6b7280; font-style: italic;">
                                    ${isStripped ? 'File stored on server - use restore button to view' : 'File removed to save storage space'}
                                </div>
                            </div>
                            ${isStripped ? '<span style="font-size: 9px; color: white; background: #059669; padding: 2px 6px; border-radius: 3px; font-weight: bold;">SERVER</span>' : ''}
                        </div>`;
                    } else if (fileData) {
                        // Display file with visual representation based on type
                        const fileIcon = this.getFileTypeIcon(filename);
                        ...
                        if (fileType === 'image' && fileData.startsWith('data:image/')) {
                            html += `<div class="message-file image-file" style="position: relative; display: inline-block; margin: 8px 0; max-width: 300px;">
                                <img src="${fileData}" alt="${filename}" style="max-width: 100%; max-height: 300px; border-radius: 8px; border: 1px solid #e5e7eb; display: block;">
                                <div style="position: absolute; top: 4px; right: 4px; background: rgba(0,0,0,0.7); color: white; padding: 2px 6px; border-radius: 4px; font-size: 10px; font-weight: 500;">
                                    ${sizeInfo.formattedSize}
                                </div>
                                <div style="position: absolute; bottom: 4px; left: 4px; background: rgba(0,0,0,0.7); color: white; padding: 2px 6px; border-radius: 4px; font-size: 10px; max-width: calc(100% - 16px); white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                                    ${filename}
                                </div>
                            </div>`;
                        }
                        // Special handling for PDFs with preview
                        else if (fileType === 'pdf') {
                            html += `<div class="message-file pdf-file" style="display: flex; align-items: stretch; padding: 12px; background: #fef2f2; border: 1px solid #fecaca; border-radius: 8px; margin: 8px 0; gap: 12px; min-width: 300px; max-width: 400px;">
                                <div style="display: flex; align-items: center; justify-content: center; width: 48px; height: 48px; background: #dc2626; border-radius: 6px; flex-shrink: 0;">
                                    <span class="material-icons" style="color: white; font-size: 24px;">picture_as_pdf</span>
                                </div>
                                <div style="flex: 1; min-width: 0;">
                                    <div style="font-size: 14px; color: #374151; font-weight: 600; margin-bottom: 4px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${filename}</div>
                                    <div style="font-size: 12px; color: #6b7280; margin-bottom: 6px;">PDF Document</div>
                                    <div style="display: flex; gap: 8px; align-items: center;">
                                        <span style="font-size: 10px; color: #dc2626; background: #fee2e2; padding: 2px 6px; border-radius: 3px; font-weight: 500;">
                                            ${sizeInfo.formattedSize}
                                        </span>
                                        <a href="${downloadUrl}" download="${filename}" style="font-size: 11px; color: #dc2626; text-decoration: none; font-weight: 500; padding: 2px 6px; border: 1px solid #dc2626; border-radius: 3px; hover: background: #dc2626; hover: color: white;">
                                            Download
                                        </a>
                                    </div>
                                </div>
                        ...
                        // Other file types with appropriate icons
                        else {
                            const { color, bgColor, borderColor } = this.getFileTypeColors(fileType);
                            html += `<div class="message-file generic-file" style="display: flex; align-items: stretch; padding: 12px; background: ${bgColor}; border: 1px solid ${borderColor}; border-radius: 8px; margin: 8px 0; gap: 12px; min-width: 300px; max-width: 400px;">
                                <div style="display: flex; align-items: center; justify-content: center; width: 48px; height: 48px; background: ${color}; border-radius: 6px; flex-shrink: 0;">
                                    <span class="material-icons" style="color: white; font-size: 24px;">${fileIcon}</span>
                                </div>
                                <div style="flex: 1; min-width: 0;">
                                    <div style="font-size: 14px; color: #374151; font-weight: 600; margin-bottom: 4px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${filename}</div>
                                    <div style="font-size: 12px; color: #6b7280; margin-bottom: 6px; text-transform: capitalize;">${fileType} File</div>
                                    <div style="display: flex; gap: 8px; align-items: center;">
                                        <span style="font-size: 10px; color: ${color}; background: ${bgColor}; padding: 2px 6px; border-radius: 3px; font-weight: 500; border: 1px solid ${borderColor};">
                                            ${sizeInfo.formattedSize}
                                        </span>
                                        <a href="${downloadUrl}" download="${filename}" style="font-size: 11px; color: ${color}; text-decoration: none; font-weight: 500; padding: 2px 6px; border: 1px solid ${color}; border-radius: 3px; transition: all 0.2s;">
                                            Download
                                        </a>
                                    </div>
                                </div>
    ...
    addMessage(role, content, index = null) {
        ...
        if (role !== 'tool') {
            // saveCurrentConversation() now handles worker-based save with fallback
            this.saveCurrentConversation();
        }
    }
    ...
    renderMessages() {
        ...
        // Apply syntax highlighting and render diagrams (with small delay for DOM updates)
        setTimeout(() => {
        ...
        try {
            ...
            while (true) {
                ...
                if (done) {
                    ...
                    // debouncedSave() → saveCurrentConversation() now handles worker-based save
                        ...
                        try {
                            ...
                            if (parsed.event_type === 'tool_call') {
                                ...
                                if (parsed.choices[0].finish_reason) {
                                    ...
                                    this.renderMessages();
                                    this.debouncedSave();
                                    // Ensure syntax highlighting and diagram rendering after streaming completes
                                    setTimeout(() => {
                                        this.highlightCodeBlocks();
                                        this.renderMermaidDiagrams();
                                        this.closeAllToolPopups();
    ...
    updateMessageContent(index, content, isStreaming = false) {
        ...
        if (messageGroups[domIndex]) {
            ...
            // Apply syntax highlighting and render diagrams when not streaming
            if (!isStreaming) {
                setTimeout(() => {
                    this.highlightCodeBlocks();
                    this.renderMermaidDiagrams();
    ...
    renderMermaidDiagrams() {
        this.waitForMermaid().then(async () => {
            // Find all mermaid diagram containers that haven't been rendered yet
            const mermaidBlocks = document.querySelectorAll('.mermaid:not([data-processed])');

            if (mermaidBlocks.length > 0) {
                console.log(`Found ${mermaidBlocks.length} mermaid diagrams to render`);

                // Render all mermaid diagrams using the correct Mermaid v10+ API
                try {
                    // Use mermaid.run() with querySelector - this will render all unprocessed mermaid elements
                    await mermaid.run({
                        querySelector: '.mermaid:not([data-processed])'
                    });

                    // Mark blocks as processed AFTER successful rendering
                    ...
                    // Don't mark as processed on error so they can be retried
                }
            }
        }).catch(error => {
    ...
    // Wait for Mermaid.js to be fully loaded
    waitForMermaid() {
        return new Promise((resolve, reject) => {
            let attempts = 0;
            ...
            const checkMermaid = () => {
                ...
                if (typeof mermaid !== 'undefined' && mermaid.run) {
                    ...
                    reject(new Error('Mermaid.js failed to load after 5 seconds'));
                } else {
    ...
    saveMermaidAsPNG(mermaidId) {
        try {
            const mermaidElement = document.getElementById(mermaidId);
            if (!mermaidElement) {
                console.error('Mermaid element not found:', mermaidId);
                return;
            }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Configure Mermaid.js if available
